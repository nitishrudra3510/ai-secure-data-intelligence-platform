...
def test_string_constraints_min_length_rejects_whitespace_only() -> None:
    """When min_length > 0, strings with only whitespace should be rejected."""
...
def test_string_constraints_min_length_zero_allows_whitespace() -> None:
    """When min_length is 0, whitespace-only strings should be allowed."""
...
def test_string_constraints_min_length_with_strip_whitespace() -> None:
    """When strip_whitespace is True, whitespace-only strings are still rejected.
    
    The whitespace validation happens at schema validation time, before any transformations
    like strip_whitespace are applied.
    """
    ta = TypeAdapter(Annotated[str, StringConstraints(min_length=1, strip_whitespace=True)])

    # String with content should work (and get stripped)
    assert ta.validate_python('  a  ') == 'a'

    # Whitespace-only strings should fail (before stripping is considered)
    with pytest.raises(ValidationError, match='too_short'):
        ta.validate_python('   ')
